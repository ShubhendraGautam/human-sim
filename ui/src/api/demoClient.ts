...
function buildMetrics(tick: number, agents: AgentColumns): SimulationMetrics {
  ...
  return {
    ...
    mean_environmental_exposure: Math.abs(season) * 0.12,
    environmental_energy_cost: Math.abs(season) * agents.id.length * 0.07,
    artifact_count: 0,
    artifact_mean_durability: 0,
    artifact_storage_capacity: 0,
    artifact_food_stored: 0,
